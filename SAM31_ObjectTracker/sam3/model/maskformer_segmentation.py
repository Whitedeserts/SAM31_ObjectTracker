...
class SegmentationHead(nn.Module):
    ...
    def _embed_pixels(
        ...
        # Multiplex features expose their dense data through .tensors. Cached
        # features can outlive a module import, so class identity is not a safe
        # discriminator here. The pixel decoder requires dense tensors only.
        backbone_feats = [getattr(feat, "tensors", feat) for feat in backbone_feats]
        for feat in backbone_feats:
            if not isinstance(feat, torch.Tensor):
                raise TypeError(
                    "Segmentation backbone features must be tensors or wrappers "
                    f"with a tensor-valued .tensors attribute; got {type(feat).__name__}"
                )

...
try:
    ...
    # triton is Linux-only and not available on Windows.
    # Define stubs so the @triton.jit decorated kernel below still parses
    # and compiles without error; edt_triton() will take the CPU path instead.
    _HAS_TRITON = False

    class _TritonDummy:
        @staticmethod
        def jit(fn=None, **kw):
            # Identity decorator - kernel will never actually be called on Windows
            if fn is not None:
                return fn
    ...
    class _TlDummy:
        constexpr = int  # absorb type annotations
...
def edt_triton(data: torch.Tensor):
    ...
    if not _HAS_TRITON or not data.is_cuda:
        # CPU fallback for Windows (triton unavailable) or CPU tensors
        import numpy as np
        device = data.device
        np_data = data.cpu().numpy().astype("uint8")
        results = []
        for b in range(np_data.shape[0]):
            from scipy.ndimage import distance_transform_edt as _scipy_edt
            dist = _scipy_edt(np_data[b] == 0).astype("float32")
